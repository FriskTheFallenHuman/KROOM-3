...
static const char* supportedExternalFormats[] =
{
	"ogg",
	"flac",
	"mp3"
};
static const int numSupportedExternalFormats = sizeof( supportedExternalFormats ) / sizeof( supportedExternalFormats[0] );
...
void idSoundSample_OpenAL::LoadResource()
{
	...
	for( int i = 0; i < 2; i++ )
	{
		...
		sampleName.Append( ".dummy" ); // GK: Just in case we get any odd file name, append a dummy file extension

		// GK:First look for ogg,mp3 and flac and then for wav
		for( int j = 0; j < numSupportedExternalFormats; j++ )
		{
...
/*
========================
idSoundSample_OpenAL::LoadAll
========================
*/
bool idSoundSample_OpenAL::LoadAll( const idStr& _name )
{
	...
	ALenum av_sample;
	int av_rate;
	bool use_avi = DecodeALAudio( &musFile, &mus_size, &av_rate, &av_sample, _name.c_str() );
	if( use_avi )
	{
		int format_byte = 0;
		bool use_ext = false;
