...
bool idSWF::HandleEvent( const sysEvent_t* event )
{
	...
	if( event->evType == SE_KEY )
	{
		...
		const char* keyName = common->KeyNumToString( ( keyNum_t )event->evValue );
		...
		for( int runaway = 0; runaway < 32; runaway++ )
		{
			...
			{
				const char* action = common->GetBinding( event->evValue );
	...
	else if( event->evType == SE_CHAR )
	{
		...
		if( focusWindow.IsObject() )
		{
			...
			if( onChar.IsFunction() )
			{
				idSWFParmList parms;
				parms.Append( event->evValue );
				parms.Append( common->KeyNumToString( ( keyNum_t )event->evValue ) );
