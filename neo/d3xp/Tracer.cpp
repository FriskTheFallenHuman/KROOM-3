...
================
*/
dnTracer::dnTracer( idEntity* owner, const float distanceRatio, const idVec3& viewOrigin, const idVec3& muzzleOrigin, const idMat3& tracerAxis )
...
================
*/
dnBeamTracer::dnBeamTracer( idEntity* owner, const float distanceRatio, const idVec3& viewOrigin, const idVec3& muzzleOrigin, const idMat3& tracerAxis )
	: dnTracer( owner )
{
	...
	// if the length is 0 then keep endPos to muzzleorigin constantly.
	if( length == 0.0f )
	{
		endPos = muzzleOrigin;
	}
...
================
*/
dnBarrelLaunchedBeamTracer::dnBarrelLaunchedBeamTracer( idEntity* owner )
	: dnTracerEffect( owner )
{
...
================
*/
dnSpeedTracer::dnSpeedTracer( idEntity* owner, const float speed, const float distance, const idVec3& muzzleOrigin, const idMat3& tracerAxis )
	: dnTracerEffect( owner )
{
...
bool dnSpeedTracer::UpdatePosition( void )
{
	...
	// if travelled all the distance, stop doing tracers.
	if( tracerTravelDist > distance )
	{
...
void dnRailBeam::Think( void )
{
	...
	if( nSmokeParticles > 0 )
	{
		...
		{
			if( !gameLocal.smokeParticles->EmitSmoke( smokeParticle, smokeStartTime, gameLocal.random.RandomFloat(), smokeOffset - smokeLength * ( i + 1 ) * renderEntity.axis[2], renderEntity.axis, 0 ) )
			{
				deadSmokeCount++;
			}
		}
