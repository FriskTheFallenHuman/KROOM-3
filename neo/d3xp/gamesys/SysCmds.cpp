...
Sets client to free camera

argv(0) freecam
==================
*/
static void Cmd_FreeCam_f( const idCmdArgs& args )
{
	// see Cmd_God_f code
	const char*		msg;
	idPlayer*	player;
...
void Cmd_Spawn_f( const idCmdArgs& args )
{
	...
	if( idStr::Icmp( value, "env_probe" ) == 0 )
	{
		...
		idStr name = gameEditLocal.GetUniqueEntityName( "env_probe_ingame_placed" );
		dict.Set( "name", name );
	}
...
void idGameLocal::InitConsoleCommands()
{
	...
	cmdSystem->AddCommand( "freecam",				Cmd_FreeCam_f,				CMD_FL_GAME | CMD_FL_CHEAT,	"enables free camera mode" );
	// RB end
