...
// the rest of the engine will only reference the "leaderBoardsLocal" variable, while all local aspects stay hidden
idLeaderboardsLocal				leaderBoardsLocal;
idLeaderboards* 				leaderBoards = &leaderBoardsLocal;	// statically pointed at an idLeaderboardsLocal
...
/*
===========
idLeaderboardsLocal::idLeaderboardsLocal
============
*/
idLeaderboardsLocal::idLeaderboardsLocal()
{
...
=====================================
*/
int idLeaderboardsLocal::GetID( int mapIndex, int gametype )
{
...
/*
=====================================
idLeaderboardsLocal::Init
=====================================
*/
void idLeaderboardsLocal::Init()
{
	...
	for( int mapIdx = 0; mapIdx < maps.Num(); mapIdx++ )
	{
		...
		for( int modeIdx = 0; modeIdx < numModes; modeIdx++ )
		{
			...
			if( maps[ mapIdx ].supportedModes & BIT( modeIdx ) )
			{
				...
				int boardID = GetID( mapIdx, modeIdx );
				...
				{
					idLib::Error( "idLeaderboardsLocal::Init leaderboard creation failed" );
				}
...
/*
=====================================
idLeaderboardsLocal::Shutdown
=====================================
*/
void idLeaderboardsLocal::Shutdown()
{
...
void idLeaderboardsLocal::Upload( lobbyUserID_t lobbyUserID, int gameType, leaderboardStats_t& stats )
{
	assert( gameType > GAME_RANDOM );
	...
	{
		idLib::Warning( "idLeaderboardsLocal::Upload invalid leaderboard with id of %d", boardID );
	}
}
...
CONSOLE_COMMAND( testLeaderboardUpload, "<gameType 0 - 4 > <frags = 0> <wins = 1>", 0 )
{
	...
	leaderBoards->Upload( user, gameType , stats );
