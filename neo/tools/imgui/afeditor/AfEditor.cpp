...
static const char* ConstraintItemGetter( void* data, int index )
{
	idDeclAF* decl = reinterpret_cast<idDeclAF*>( data );
	if( !decl )
	{
...
void AfEditor::Draw()
{
	...
	if( ImGui::Begin( "###ArticulatedFigureEditorToolBar", &showTool, outerFlags ) )
	{
		...
		if( ImGui::BeginPopupModal( "New Articulated Figure", NULL, ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoTitleBar ) )
		{
			if( afFiles.Num() == 0 )
			{
		...
		if( ImGui::BeginPopupModal( "Articulated Figure Browser", NULL, ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoTitleBar ) )
		{
			afList.populate();
...
void AFEditorInit()
{
	...
	AfEditor::Instance().ShowIt( true );
	imguiSystem->GetEditor()->RegisterWindow( AfEditor::Instance() );
	imguiSystem->GetEditor()->ReleaseMouse( true );
	imguiSystem->RegisterDockWindow( "###ArticulatedFigureEditor", DOCK_REGION_LEFT );
}