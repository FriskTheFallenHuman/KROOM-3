...
ParticleEditor::ParticleEditor()
	...
	, useSnap( false )
	, gridSnap{ 8.0f, 8.0f, 8.0f }
	, angleSnap( 15.0f )
	, gizmoOrigin( vec3_origin )
...
void ParticleEditor::Draw()
{
	...
	if( ImGui::Begin( "###ParticleEditorToolBar", &showTool, flags ) )
	{
		...
		if( ImGui::BeginMainMenuBar() )
		{
			...
			{
				if( ImGui::MenuItem( "Model", "Alt+0" ) )
				{
					visualization = TESTMODEL;
					updateParticleView = true;
				}

				if( ImGui::IsItemHovered() )
				{
					ImGui::SetTooltip( "Show the selected particle as a testmodel" );
				}

				if( ImGui::MenuItem( "Projectiles", "Alt+1" ) )
				{
				...
				if( ImGui::IsItemHovered() )
				{
					ImGui::SetTooltip( "Show the selected particle as projectile impacts" );
				}

				if( ImGui::MenuItem( "Muzzle Flash", "Alt+2" ) )
				{
				...
				if( ImGui::MenuItem( "Projectile Flight Smoke", "Alt+3" ) )
				{
				...
				if( ImGui::IsItemHovered() )
				{
					ImGui::SetTooltip( "Show the selected particle as projectile flight smoke" );
				}

				if( ImGui::MenuItem( "Selected Entity", "Alt+4" ) )
				{
				...
				if( ImGui::IsItemHovered() )
				{
					ImGui::SetTooltip( "Show the selected particle on the selected entity" );
				}

				ImGui::EndMenu();
			}

			if( ImGui::BeginMenu( "Entity Editing" ) )
			{
				ImGui::BeginDisabled( !editControlsEnabled );

				if( ImGui::MenuItem( "Translate", nullptr, mCurrentGizmoOperation == ImGuizmo::TRANSLATE ) )
				{
					mCurrentGizmoOperation = ImGuizmo::TRANSLATE;
				}

				if( ImGui::MenuItem( "Rotate", nullptr, mCurrentGizmoOperation == ImGuizmo::ROTATE ) )
				{
				...
				ImGui::MenuItem( "Snap", nullptr, &useSnap );
		...
		if( ImGui::Begin( "Particle Editor###ParticleEditor", &showTool, contentFlags ) )
		{
			...
			if( ImGui::CollapsingHeader( "Stages" ) )
			{
				...
				if( ImGui::CollapsingHeader( "Timing / Ammount" ) )
				{
			...
			if( ImGui::CollapsingHeader( "Direction / Orientation" ) )
			{
				ImGui::BeginDisabled( !stageControlsEnabled );
			...
			if( ImGui::CollapsingHeader( "Speed / Rotation / Size / Aspect" ) )
			{
				ImGui::BeginDisabled( !stageControlsEnabled );
...
void ParticleEditor::OnBnClickedParticleMode()
{
	...
	gameEdit->PlayerEnableFreeCam( particleMode );
	EnableEditControls();
}
...
void ParticleEditor::UpdateParticleData()
{
	...
	{
		int index = listStages.Append( va( "stage %i", i ) );
		if( index >= 0 )
		{
	...
	listStagesSel = 0;
	OnLbnSelchangeListStages();
	inFileText = va( "%s (%s)", idp->GetName(), idp->GetFileName() );
...
void ParticleEditor::AddStage( bool clone )
{
	...
	int index = listStages.Append( va( "stage %i", newIndex ) );
	listStagesSel = index;
...
void ParticleEditor::ShowStage()
{
	...
	listStages.RemoveIndex( index );
	listStagesItemData.Remove( index, newIndex );
	listStages.Insert( va( "stage %i", index ), index );
	listStagesItemData.Add( index, newIndex );
	listStagesSel = index;
	EnableStageControls();
}
...
void ParticleEditor::HideStage()
{
	...
	listStages.RemoveIndex( index );
	listStagesItemData.Remove( index, newIndex );
	listStages.Insert( va( "stage %i (H) ", index ), index );
	listStagesItemData.Add( index, newIndex );
	listStagesSel = index;
	EnableStageControls();
}
...
// static
void ParticleEditor::ReInit( const idDict* dict, idEntity* entity )
{
	ParticleEditor& editor = Instance();

	if( dict != NULL && entity != NULL )
	{
		gameEdit->ClearEntitySelection();
		gameEdit->AddSelectedEntity( entity );

		const char* model = dict->GetString( "model" );
		if( model[0] != '\0' )
		{
			editor.SelectParticle( model );
		}
	}
...
void ParticleEditor::DrawGizmo()
{
	...
	if( !ImGuizmo::IsUsing() )
	{
		gameEdit->EntityGetOrigin( list[0], gizmoOrigin );
		gameEdit->EntityGetAxis( list[0], gizmoAxis );
	}
	...
	if( ImGui::Begin( "###ParticleEditorGizmoHost", NULL, flags ) )
	{
		ImGuizmo::SetRect( 0.0f, 0.0f, io.DisplaySize.x, io.DisplaySize.y );
		ImGuizmo::SetOrthographic( false );
		ImGuizmo::SetDrawlist();
		ImGuizmo::SetID( 2 ); // 0 = LightEditor, 1 = SoundEditor - keep gizmo IDs distinct

		idMat4 gizmoMatrix( gizmoAxis, gizmoOrigin );
		idMat4 manipMatrix = gizmoMatrix.Transpose();

		const float* snap = useSnap ? ( mCurrentGizmoOperation == ImGuizmo::TRANSLATE ? gridSnap : &angleSnap ) : NULL;

		ImGuizmo::Manipulate( viewDef.worldSpace.modelViewMatrix, viewDef.projectionMatrix,
							  mCurrentGizmoOperation, ImGuizmo::WORLD, manipMatrix.ToFloatPtr(), NULL, snap );

		if( ImGuizmo::IsUsing() )
		{
			idMat4 result = manipMatrix.Transpose();

			if( mCurrentGizmoOperation == ImGuizmo::TRANSLATE )
			{
				idVec3 delta = result.GetTranslation() - gizmoOrigin;
				UpdateSelectedOrigin( delta.x, delta.y, delta.z );
				gizmoOrigin = result.GetTranslation();
			}
			else if( mCurrentGizmoOperation == ImGuizmo::ROTATE )
			{
				gizmoAxis = result.ToMat3();
				SetVectorControlUpdate( gizmoAxis.ToQuat() );
			}
		}
	}
	ImGui::End();
...
void ParticleEditorInit( const idDict* spawnArgs, idEntity* ent )
{
	...
	ParticleEditor::ReInit( spawnArgs, ent );
	ParticleEditor::Instance().ShowIt( true );
	imguiSystem->GetEditor()->RegisterWindow( ParticleEditor::Instance() );
	imguiSystem->GetEditor()->ReleaseMouse( true );
	imguiSystem->RegisterDockWindow( "###ParticleEditor", DOCK_REGION_LEFT );
