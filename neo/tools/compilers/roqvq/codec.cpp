...
codec::codec()
{
	...
	for( i = 0; i < 256; i++ )
	{
		codebook2[i] = ( VQDATA* ) Mem_ClearedAlloc( 16 * sizeof( VQDATA ), TAG_ROQ );
	}
	codebook4 = ( VQDATA** ) Mem_ClearedAlloc( 256 * sizeof( VQDATA* ), TAG_ROQ );
	for( i = 0; i < 256; i++ )
	{
		codebook4[i] = ( VQDATA* ) Mem_ClearedAlloc( 64 * sizeof( VQDATA ), TAG_ROQ );
	}
	previousImage[0] = 0;
	previousImage[1] = 0;
	image = 0;
...
void codec::Segment( int* alist, float* flist, int numElements, float rmse )
{
	...
	if( doopen )
	{
		...
		cbook = ( byte* )Mem_ClearedAlloc( 3 * image->pixelsWide() * image->pixelsHigh(), TAG_ROQ );
		float* snrBook = ( float* )Mem_ClearedAlloc( image->pixelsWide() * image->pixelsHigh(), TAG_ROQ );
		dst = cbook;
		int numEntries = 0;
...
void codec::MakePreviousImage( quadcel* pquad )
{
	...
	idataA = ( byte* )Mem_Alloc( 16 * 16 * 4, TAG_ROQ );
...
void codec::InitImages()
{
	...
	qStatus = ( quadcel* )Mem_ClearedAlloc( numQuadCels * sizeof( quadcel ), TAG_ROQ );
	InitQStatus();
//
	if( previousImage[0] )
	{
		...
		if( !luti )
		{
			luti = ( byte* )Mem_Alloc( pixelsWide * pixelsHigh, TAG_ROQ );
		}
...
int codec::AddQuad( quadcel* pquad, int lownum )
{
	...
	if( lownum != -1 )
	{
		...
		newsnr = 0;
		idataA = ( byte* )Mem_Alloc( 8 * 8 * 4, TAG_ROQ );
		idataB = ( byte* )Mem_Alloc( 8 * 8 * 4, TAG_ROQ );
...
void codec::SparseEncode()
{
	...
	idataA = ( byte* )Mem_Alloc( 16 * 16 * 4, TAG_ROQ );
	idataB = ( byte* )Mem_Alloc( 16 * 16 * 4, TAG_ROQ );
	...
	flist = ( float* )Mem_ClearedAlloc( ( numQuadCels + 1 ) * sizeof( float ), TAG_ROQ );
	ilist = ( int* )Mem_ClearedAlloc( ( numQuadCels + 1 ) * sizeof( int ), TAG_ROQ );
