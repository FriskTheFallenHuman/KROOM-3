...
void	FixAreaGroupsTjunctions( optimizeGroup_t* groupList )
{
	...
	DMAP_VERBOSITY( "----- FixAreaGroupsTjunctions -----\n" );
	DMAP_VERBOSITY( "%6i triangles in\n", startCount );
	...
	DMAP_VERBOSITY( "%6i triangles out\n", endCount );
}
...
void	FixGlobalTjunctions( uEntity_t* e )
{
	...
	DMAP_VERBOSITY( "----- FixGlobalTjunctions -----\n" );
	...
	if( dmapGlobals.entityNum == 0 )
	{
		for( int eNum = 1 ; eNum < dmapGlobals.num_entities ; eNum++ )
		{
			...
			// RB: OBJ support
			if( !strstr( modelName, ".lwo" ) && !strstr( modelName, ".ase" ) && !strstr( modelName, ".ma" ) && !strstr( modelName, ".md3" ) && !strstr( modelName, ".obj" ) )
			{
				continue;
			}
