...
void ClipSidesByTree( uEntity_t* e )
{
	...
	DMAP_VERBOSITY( "----- ClipSidesByTree -----\n" );
...
void PutPrimitivesInAreas( uEntity_t* e )
{
	...
	DMAP_VERBOSITY( "----- PutPrimitivesInAreas -----\n" );
...
void FilterMeshesIntoTree( uEntity_t* e )
{
	...
	DMAP_VERBOSITY( "----- FilterMeshesIntoTree -----\n" );
	...
	DMAP_VERBOSITY( "%5i total BSP triangles\n", c_unique );
	DMAP_VERBOSITY( "%5i cluster references\n", c_clusters );
}
// RB end
...
void Prelight( uEntity_t* e )
{
	...
	if( dmapGlobals.shadowOptLevel > 0 )
	{
		...
		DMAP_VERBOSITY( "%5.1f seconds for BuildLightShadows\n", ( end - start ) / 1000.0 );
	}


	if( !dmapGlobals.noLightCarve )
	{
		DMAP_VERBOSITY( "----- CarveGroupsByLight -----\n" );
		start = Sys_Milliseconds();
		...
		DMAP_VERBOSITY( "%5.1f seconds for CarveGroupsByLight\n", ( end - start ) / 1000.0 );
	}
