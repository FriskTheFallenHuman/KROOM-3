...
void idRenderBackend::BindVariableStageImage( const textureStage_t* texture, const float* shaderRegisters )
{
	if( texture->cinematic )
	{
		...
		cin = texture->cinematic->ImageForTime( viewDef->renderView.time[1] + idMath::Ftoi( 1000.0f * viewDef->renderView.shaderParms[11] ) );
		if( cin.imageY != NULL )
		{
			GL_SelectTexture( 0 );
			cin.imageY->Bind();
...
void idRenderBackend::ShadowMapPass( const drawSurf_t* drawSurfs, const viewLight_t* vLight, int side )
{
	...
	switch( r_shadowMapOccluderFacing.GetInteger() )
	{
		case 0:
			...
			GL_PolygonOffset( r_shadowMapPolygonFactor.GetFloat(), r_shadowMapPolygonOffset.GetFloat(), r_shadowMapPolygonClamp.GetFloat() );
			break;
...
void idRenderBackend::DrawInteractions( const viewDef_t* _viewDef )
{
	...
	for( const viewLight_t* vLight = viewDef->viewLights; vLight != NULL; vLight = vLight->next )
	{
		...
		if( vLight->localInteractions != NULL )
		{
			...
			RenderInteractions( vLight->localInteractions, vLight, GLS_DEPTHFUNC_EQUAL, false, false );
			renderLog.CloseBlock();
		}
		...
		if( vLight->globalInteractions != NULL )
		{
			...
			RenderInteractions( vLight->globalInteractions, vLight, GLS_DEPTHFUNC_EQUAL, false, false );
			renderLog.CloseBlock();
		}
